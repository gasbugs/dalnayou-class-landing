...
클씨랩 Facebook 페이지 첫 게시물:

```text
Gemini 노트북:
https://gasbugs.github.io/dalnayou-class-landing/notebooklm.html?utm_source=facebook_page&utm_medium=organic_social&utm_campaign=dalnayou_2026_08&utm_content=combined_launch_post_notebooklm

로블록스 AI:
https://gasbugs.github.io/dalnayou-class-landing/roblox.html?utm_source=facebook_page&utm_medium=organic_social&utm_campaign=dalnayou_2026_08&utm_content=combined_launch_post_roblox
```

클씨랩 Facebook 페이지 Gemini 업무 자료 후속 게시물:
...
LinkedIn 첫 B2C 클래스 소개:

```text
Gemini 노트북:
https://gasbugs.github.io/dalnayou-class-landing/notebooklm.html?utm_source=linkedin&utm_medium=organic_social&utm_campaign=dalnayou_2026_08&utm_content=first_b2c_notebooklm

로블록스 AI:
https://gasbugs.github.io/dalnayou-class-landing/roblox.html?utm_source=linkedin&utm_medium=organic_social&utm_campaign=dalnayou_2026_08&utm_content=first_b2c_roblox
```
...
`application_submit`은 기존 Google 신청서와 연결된 응답 스프레드시트의 Apps Script 설치형 트리거로 전송합니다. 트리거는 `trackApplicationSubmit` 함수를 `스프레드시트에서 → 양식 제출 시` 조건으로 실행합니다. 강좌 페이지는 신청 링크의 `유입 정보 (자동 입력)` 항목에 UTM·랜딩 경로·모집 단계·현재 가격·CTA 위치를 미리 채우고, Apps Script는 이를 허용 목록으로 제한해 GA4에 전달합니다. 이름·전화번호 등 개인정보는 GA4로 보내지 않습니다.

2026년 7월 25일부터 `application_submit`에도 `link_position`이 전달됩니다. 따라서
`notebooklm_hero_form`, `notebooklm_enterprise_trust_form`,
`notebooklm_final_form`, `notebooklm_mobile_form`과 Roblox의 대응 위치를 실제 제출
기준으로 비교할 수 있습니다. Apps Script 원본은
`integrations/google-apps-script/track-application-submit.gs`에 보관하며, 라이브
프로젝트 `달나유 GA4 신청 완료 추적`의 `Code.gs`와 동기화합니다.
...
- `[Codex] 2026-08_AI클래스_모집` 캠페인에서 과정별 광고 세트 2개 운영
- 지역은 소사역 중심 약 8km 통학 가능 범위
- Meta 직접 노출 연령은 18~44세
- Gemini 노트북과 로블록스 AI에 각각 일 예산 `20,000원` 배정
- 전체 일 예산 상한은 `40,000원`
- 각 광고는 해당 과정 상세 페이지로 직접 연결
- 현재 긴급 모집에서 과정별 입금 6명은 개강 기준이며 광고는 계속 운영
- 입금 15명 정원에 도달한 과정의 광고 세트만 중단
...
2026년 7월 26일 Search Console URL 접두어 속성을 GA4 웹 스트림
`dalnayou-class-landing`(스트림 ID `15201481192`)에 연결했습니다. 검색어와
Google 자연 검색 방문 페이지 보고서는 연결 이후 데이터 처리에 시간이 걸릴 수
있으므로, 데이터가 생성된 뒤 Search Console 보고서에서 확인합니다.
...
`ga4-events.js`와 `marketing-events.js`는 `localhost`, `127.0.0.1`, `[::1]`에서
외부 추적을 전송하지 않습니다. 2026년 7월 26일 10:22~10:28 KST에 이 보호
장치를 추가하기 전 Facebook UTM으로 수행한 로컬 QA가 있으므로, 해당 시간대
보고서는 호스트 이름이 실제 공개 도메인인 방문만 사용합니다.
