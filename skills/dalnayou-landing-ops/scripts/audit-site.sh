...
contains scripts/analyze-marketing-funnel.mjs 'paymentConfirmed >= capacity' 'Marketing analyzer stops a course only at configured capacity'
contains scripts/analyze-marketing-funnel.mjs 'minimumEnrollment.*개강 기준.*capacity.*광고 중단 기준' 'Marketing report distinguishes minimum enrollment from capacity'
